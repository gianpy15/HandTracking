...
from library.neural_network.keras.models.joints import finger_field_injection
...
from library.utils.visualization_utils import joint_skeleton_impression, finger_field_impression
...
model = 'transfer_vector_field_jlocator'
...
# how much the vector field inference loss is scaled up against the visibility loss.
# makes sure that the algorithm gives the right priority to losses
vector_field_priority = 5
...
if __name__ == '__main__':
    ...
    heatmap_loss = lambda x, y: heat_priority*prop_heatmap_penalized_fp_loss(x, y,
                                                                             ...
                                                                             delta=delta)
    vector_field_loss = lambda x, y: vector_field_priority * tf.norm(x-y, ord=2)
    ...
    formatting = format_add_outer_func(f=reg_3.apply,
                                       format=formatting,
                                       entry=OUT('field'))
    ...
    model = train_model(model_generator=lambda: finger_field_injection( # input_shape=np.shape(dm.train()[0][IN('img')][0]),
                                                                          dropout_rate=drate,
                                                                          train_mobilenet=retrain,
                                                                          # activation=K.layers.LeakyReLU(alpha=leaky_slope)
                                                                          ),
                        dataset_manager=dm,
                        loss={OUT('heat'): heatmap_loss,
                              OUT('field'): vector_field_loss,
