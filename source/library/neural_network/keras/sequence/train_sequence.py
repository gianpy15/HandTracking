...
class TrainSequence(Sequence):
    def __init__(self, data_generator: DatasetManager,
        ...
        """
        super(TrainSequence, self).__init__()
        self.epoch = 0
        ...
        self.current_batch = self.__augment_and_regularize(self.data_generator.get_training_batch())

    def __getitem__(self, index):
        batch = self.current_batch
        del self.current_batch
        self.augmenting_batch = self.pool.submit(
            self.__augment_and_regularize(self.data_generator.get_training_batch()))
        return batch

    def __len__(self):
        return self.data_generator.get_training_batch_number()
    ...
    def __augment_data(self, batch):
        return self.augmenter.apply_on_batch(batch) if self.augmenter is not None else batch
