...
def unsequential_model():
    ...
    x = klayers.Conv2D(filters=120, kernel_size=[3, 3])(x)
    x = klayers.Conv2D(filters=170, kernel_size=[3, 3])(x)
    ...
    x = klayers.MaxPool2D(pool_size=[2, 2])(x)
    x = klayers.Conv2D(filters=250, kernel_size=[3, 3])(x)
    x = klayers.Conv2D(filters=250, kernel_size=[3, 3])(x)
    x = klayers.Conv2D(filters=400, kernel_size=[1, 1])(x)
    x = klayers.Flatten()(x)
    x = klayers.Dense(units=30, activation='relu', use_bias=True)(x)
