...
def simple_classifier_rgb(weight_decay=None):
    ...
    x = klayers.Conv2D(filters=100, kernel_size=[3, 3], activation='relu')(inputs)

    x = klayers.Conv2D(filters=110, kernel_size=[3, 3], activation='relu')(x)
    ...
    x = klayers.Conv2D(filters=140, kernel_size=[3, 3], activation='relu')(x)
