...
def simple_classifier_rgb(weight_decay=None):
    ...
    x = klayers.Conv2D(filters=50, kernel_size=[3, 3], activation='relu')(inputs)

    x = klayers.Conv2D(filters=60, kernel_size=[3, 3], activation='relu')(x)

    x = klayers.Conv2D(filters=60, kernel_size=[3, 3], activation='relu')(x)

    x = klayers.MaxPooling2D(pool_size=[2, 2])(x)

    x = klayers.Conv2D(filters=70, kernel_size=[3, 3], activation='relu')(x)
