...
def eta_net(input_shape, weight_decay=None, name='u_net', dropout_rate=0, activation='relu'):
    ...
    conv6 = kl.Conv2D(filters=256, kernel_size=[3, 3], activation='relu', padding='same')(merge6)
    act6 = kl.Activation(activation)(conv6) if type(activation) is str else activation()(conv6)
    conv6 = kl.Conv2D(filters=256, kernel_size=[3, 3], activation='relu', padding='same')(act6)
    act6 = kl.Activation(activation)(conv6) if type(activation) is str else activation()(conv6)

    up7 = kl.Conv2D(filters=128, kernel_size=[3, 3], padding='same')(kl.UpSampling2D(size=[2, 2])(act6))
    ...
    conv7 = kl.Conv2D(filters=128, kernel_size=[3, 3], activation='relu', padding='same')(merge7)
