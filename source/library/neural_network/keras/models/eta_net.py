...
def eta_net(input_shape, weight_decay=None, name='u_net', dropout_rate=0, activation='relu'):
    ...
    conv5 = kl.Conv2D(filters=256, kernel_size=[3, 3], padding='same')(pool4)
    act5 = kl.Activation(activation)(conv5) if type(activation) is str else activation()(conv5)
    conv5 = kl.Conv2D(filters=256, kernel_size=[3, 3], padding='same')(act5)
    act5 = kl.Activation(activation)(conv5) if type(activation) is str else activation()(conv5)
    ...
    conv6 = kl.Conv2D(filters=256, kernel_size=[3, 3], padding='same')(merge6)
    act6 = kl.Activation(activation)(conv6) if type(activation) is str else activation()(conv6)
    conv6 = kl.Conv2D(filters=256, kernel_size=[3, 3], padding='same')(act6)
    act6 = kl.Activation(activation)(conv6) if type(activation) is str else activation()(conv6)
    ...
    conv7 = kl.Conv2D(filters=128, kernel_size=[3, 3], padding='same')(merge7)
    act7 = kl.Activation(activation)(conv7) if type(activation) is str else activation()(conv7)
    conv7 = kl.Conv2D(filters=128, kernel_size=[3, 3], padding='same')(act7)
    act7 = kl.Activation(activation)(conv7) if type(activation) is str else activation()(conv7)
