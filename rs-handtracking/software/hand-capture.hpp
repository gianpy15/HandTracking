// Code adapted from examples.hpp
...
public:
    ...
    void showAndSave(rs::device & dev, rs::stream stream, int rx, int ry, int rw, int rh, const char* filename)
    {
        if((stream <= rs::stream::fisheye) && (!dev.is_stream_enabled(stream))) return;

        uploadAndSave(dev, stream, filename);
    ...
    void uploadAndSave(rs::device & dev, rs::stream stream, const char* filename)
    {
        FILE* fp;
        if (stream <= rs::stream::fisheye)
            assert(dev.is_stream_enabled(stream));
        ...
        if(timestamp != last_timestamp)
        {
            const void * frame = dev.get_frame_data(stream);

            if(fp=fopen(filename, "wb")){
                switch(dev.get_stream_format(stream)){
                    case rs::format::z16:
                        fwrite(frame, 2, dev.get_stream_width(stream)*dev.get_stream_height(stream), fp);
                        break;
                    case rs::format::rgb8:
                        fwrite(frame, 1, 3*dev.get_stream_width(stream)*dev.get_stream_height(stream), fp);
                        break;
                }
                fclose(fp);
            }

            upload(frame, dev.get_stream_width(stream), dev.get_stream_height(stream), dev.get_stream_format(stream));
            last_timestamp = timestamp;
	...
	void showAndSend(rs::device & dev, rs::stream stream, int rx, int ry, int rw, int rh, int socket)
    {
        if((stream <= rs::stream::fisheye) && (!dev.is_stream_enabled(stream))) return;

        uploadAndSend(dev, stream, socket);
    ...
    void uploadAndSend(rs::device & dev, rs::stream stream, int socket)
    {
        if (stream <= rs::stream::fisheye)
            assert(dev.is_stream_enabled(stream));
        ...
        if(timestamp != last_timestamp)
        {
            ...
            switch(dev.get_stream_format(stream)){
                case rs::format::z16:
                    sendFrame(socket, frame, 2*dev.get_stream_width(stream)*dev.get_stream_height(stream));
                    break;
                case rs::format::rgb8:
                    sendFrame(socket, frame, 3*dev.get_stream_width(stream)*dev.get_stream_height(stream));
                    break;
            }
            upload(frame, dev.get_stream_width(stream), dev.get_stream_height(stream), dev.get_stream_format(stream));
            last_timestamp = timestamp;
	...
	void getAndSend(rs::device & dev, rs::stream stream, int socket)
	{
        if (stream <= rs::stream::fisheye)
            assert(dev.is_stream_enabled(stream));
